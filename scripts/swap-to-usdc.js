...
/**
 * Swap ETH to USDC on Base via Uniswap V3. The bot needs USDC on Base
 * to pay Neynar's hub via x402 micropayments.
 *
 * Defaults to swapping 1/3 of the wallet's ETH balance, which is plenty
 * for thousands of casts.
 *
 * @param {string} privateKey - Wallet private key
 * @param {bigint} [amountIn] - ETH to swap (wei). Defaults to balance/3.
...
async function swapEthToUsdc(privateKey, amountIn = null) {
  ...
  // Try the 0.05% pool first (typical for ETH/stable), fall back to 0.3%
  ...
  for (const fee of feeTiers) {
    try {
      ...
      return { usdcReceived: formatUnits(usdcReceived, 6), txHash: tx.hash };
    } catch (e) {
...
if (require.main === module) {
  ...
  if (!privateKey) {
    console.log('Usage: PRIVATE_KEY=0x... node scripts/swap-to-usdc.js');
    process.exit(1);
  }
